...
public:
    ...
    /**
     * Schedules the execution of a "Get Data" command to retrieve the data indicated by the
     * provided tag.
     *
     * <p>Data will be available in {@link CalypsoCard} using the {@link ElementaryFile#getHeader()}
     ...
     * session. Instead, use the method {@link #prepareReadRecords(byte, int, int, int)} for this
     ...
     * @throw IllegalStateException If this method is called inside a secure session in contact
     *        mode.
     * @since 1.1.0
     */
     ...
     * <ul>
     *   <li>Outside a secure session (best effort mode): the following "process" command will not
     *       fail whatever the existence of the targeted file or the validity of the offset and
     *       number of bytes to read (the {@link CalypsoCard} object may not be filled).
     *   <li>Inside a secure session (strict mode): the following "process" command will fail if the
     ...
     *       object may not be filled).
     *   <li>Inside a secure session (strict mode): the following "process" command will fail if the
     *       targeted file or counter does not exist (the {@link CalypsoCard} object is always
    ...
    /**
     * Schedules the execution of a "Search Record Multiple" command to search data in the records
     * of the indicated EF, from a given record to the last record of the file. It will return the
     * list of record numbers containing these data, and if requested it will read the first record
     * content.
     *
     * <p>The command is only possible with a "linear", "cyclic", Counters or Simulated "counter"
     * EF.
     *
     ...
     * SearchCommandData object, and the content of the first matching record in CalypsoCard if
     * requested.
     *
     * <p>Depending on whether we are inside a secure session, there are two types of behavior
     * following this command:
     ...
     *       and offset (the {@link SearchCommandData} and {@link CalypsoCard} objects may not be
     *       updated).
     *   <li>Inside a secure session (strict mode): the following "process" command will fail if the
    ...
    virtual T& prepareUpdateRecord(
        const uint8_t sfi, const int recordNumber, const std::vector<uint8_t>& recordData)
        = 0;
     ...
     */
    virtual T&
    prepareUpdateBinary(const uint8_t sfi, const int offset, const std::vector<uint8_t>& data)
        = 0;

    /**
     * Schedules the execution of one or multiple "Write Binary" commands to write over the
     * indicated data of a "binary" file. The new data will be the result of a binary OR operation
     * between the existing data and the data given in the command from the indicated offset.
     *
     * <p>The data of the file before the offset and after the data given are left unchanged.
     *
     * <p>Data will be available in {@link CalypsoCard} using the dedicated file and data management
     ...
     */
    virtual T&
    prepareWriteBinary(const uint8_t sfi, const int offset, const std::vector<uint8_t>& data)
        = 0;
     ...
     */
    virtual T&
    prepareIncreaseCounter(const uint8_t sfi, const int counterNumber, const int incValue)
        = 0;
    ...
    virtual T& prepareIncreaseCounters(
        const uint8_t sfi, const std::map<const int, const int>& counterNumberToIncValueMap)
        = 0;
     ...
     */
    virtual T&
    prepareDecreaseCounter(const uint8_t sfi, const int counterNumber, const int decValue)
        = 0;
    ...
    virtual T& prepareDecreaseCounters(
        const uint8_t sfi, const std::map<const int, const int>& counterNumberToDecValueMap)
        = 0;
     ...
     * counter.
     *
     * <p>The operation (Increase or Decrease) is selected according to whether the difference
     * between the current value and the desired value is negative (Increase) or positive
     * (Decrease).
     *
     * <p>Data will be available in {@link CalypsoCard} using the dedicated file and data management
     * methods.
     ...
     * @since 1.0.0
     */
    virtual T& prepareSetCounter(const uint8_t sfi, const int counterNumber, const int newValue)
        = 0;
    ...
    /**
     * Schedules the execution of a "Change PIN" command to replace the current PIN with the new
     * value provided.
     *
     ...
     * PIN command (see {@link #prepareVerifyPin(byte[])}).
     *
     * <p>The PIN status will be available in {@link CalypsoCard} using the {@link
     * CalypsoCard#getPinAttemptRemaining()} and {@link CalypsoCard#isPinBlocked()} methods.
     *
     ...
     *
     * <p>For write commands, the {@link CalypsoCard} is updated only when the command is
     * successful.
     *
     * <p>The process is interrupted at the first failed command.
     *
     * @param channelControl Policy for managing the physical channel after executing commands to
     *        the card.
     * @return The current instance.
     * @throw ReaderIOException If a communication error with the card reader or the cryptographic
     *        module reader occurs.
     * @throw CardIOException If a communication error with the card occurs.
     * @throw CryptoIOException If a communication error with the cryptographic module occurs.
     ...
     * @throw InvalidCardSignatureException If the card signature is incorrect. In the case of a
     *        card transaction secured by "symmetrical" cryptography (e.g. SAM), this indicates that
     *        the card has correctly closed the secure session, but the card session is not
     *        authentic because the MAC of the card is incorrect.
