...
public:
     ...
     * Record" command.
     *
     * <p>By default, this optimization is performed when the command that follows the session
     * opening is a "Read Record" command.
     *
    ...
    assignKif(const WriteAccessLevel writeAccessLevel, const uint8_t kvc, const uint8_t kif)
        = 0;
    ...
    /**
     * Defines for a given write access level the KVC value to use for cards that do not provide
     * KVC.
     *
     ...
     * @since 1.0.0
     */
    virtual SymmetricCryptoSecuritySetting& addAuthorizedSvKey(const uint8_t kif, const uint8_t kvc)
        = 0;
    ...
    /**
     * Prepares the cryptographic module for the next transaction by anticipating all security
     * context configuration operations.
     *
     * <p>This feature is only useful if the currently allocated cryptographic module will be used
     * for the next transaction. It is particularly relevant to optimize the transaction time in a
     ...
     *
     * @throw CryptoException If an error occurred when computing a crypto operation.
     * @throw CryptoIOException If a communication error with the crypto module (e.g. timeout with
