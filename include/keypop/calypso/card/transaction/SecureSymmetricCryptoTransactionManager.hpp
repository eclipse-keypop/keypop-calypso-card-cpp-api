...
/**
 * Contains operations common to all card transactions secured by "symmetric"
 * key cryptographic algorithms.
 *
 * <p>See {@link SecureTransactionManager} parent interface for more information
 * and details of available card operations.
 *
...
public:
     ...
     *
     * <p>The secure session will be opened with the provided {@link
     * WriteAccessLevel} depending on whether it is a personalization, reload or
     ...
     * Secure Session" command for optimization purposes except if the
     * "pre-open" mode is active.
     *
     * <p>This mechanism may in some cases be incompatible with the security
     * constraints and can be disabled via the {@link
     * SymmetricCryptoSecuritySetting#disableReadOnSessionOpening()} method.
     *
     ...
     * @since 1.6.0
     */
    virtual T& prepareOpenSecureSession(const WriteAccessLevel writeAccessLevel)
        = 0;

    /**
     * Schedules the execution of a "SV Get" command to prepare an SV operation
     * or simply retrieves the current SV status.
     *
     * <p>Data will be available in {@link CalypsoCard} using the dedicated SV
     * data management methods.
     *
     * @param svOperation Informs about the nature of the intended operation:
     * debit or reload.
     * @param svAction The type of action: DO a debit or a positive reload, UNDO
     * an undebit or a negative reload.
     * @return The current instance.
     ...
     * SV balance and using the provided additional data.
     *
     * <p>Data will be available in {@link CalypsoCard} using the dedicated SV
     * data management methods.
     *
     ...
     *
     * @param amount The value to be reloaded, positive or negative integer in
     ...
     *     <ul>
     *       <li>Another SV command was already prepared inside the same secure
     * session. <li>The SV command is not placed in the first position in the
     * list of prepared commands. <li>The SV command does not follow a "SV Get"
     * command. <li>The command and the SV operation are not consistent.
     *     </ul>
     *
     ...
     *
     * @param amount The value to be reloaded, positive integer in the range
     * 0..8388607 for a DO. action, in the range 0..8388608 for an UNDO action.
     * @return The current instance.
     ...
     * increase the current SV balance or to partially or totally cancels the
     * last SV debit command and using the provided additional data.
     *
     * <p>It consists in decreasing the current balance of the SV by a certain
     * amount or canceling a previous debit according to the type operation
     * chosen in when invoking the previous SV Get command.
     *
     * <p>Data will be available in {@link CalypsoCard} using the dedicated SV
     * data management methods.
     *
     ...
     *
     * @param amount The amount to be subtracted or added, positive integer in
     * the range 0..32767 when subtracted and 0..32768 when added.
     * @param date 2-byte free value.
     * @param time 2-byte free value.
     * @return The current instance.
     * @throw IllegalArgumentException If one of the provided argument is out of
     ...
     * Schedules the execution of a "SV Debit" or "SV Undebit" command to
     * increase the current SV balance or to partially or totally cancels the
     * last SV debit command.
     *
     * <p>It consists in decreasing the current balance of the SV by a certain
     * amount or canceling a previous debit.
     *
     * <p>Data will be available in {@link CalypsoCard} using the dedicated SV
     * data management methods.
     *
     ...
     *
     * <p>This command is usually executed within a secure session with the
     * DEBIT key (depends on the access rights given to this command in the file
     * structure of the card).
     *
     * <p>The DF status will be available in {@link CalypsoCard} using the
     * {@link CalypsoCard#isDfInvalidated()} method.
     *
     ...
     *
     * <p>This command is usually executed within a secure session with the
     * PERSONALIZATION key (depends on the access rights given to this command
     * in the file structure of the card).
     *
     * <p>The DF status will be available in {@link CalypsoCard} using the
     * {@link CalypsoCard#isDfInvalidated()} method.
     *
    ...
    /**
     * Schedules the execution of "Change Key" command to replace one of the
     * current card keys with another key present in the cryptographic module.
     *
     * <p>This command can be performed only out of a secure session.
     *
     * <p>The change key process transfers the key from the cryptographic module
     * to the card. The new key is diversified by the cryptographic module from
     * a primary key and encrypted using the indicated issuer key to secure the
     * transfer to the card. All provided KIFs and KVCs must be present in the
     * cryptographic module.
     *
     * @param keyIndex The index of the key to be replaced (1 for the issuer
     * key, 2 for the load key, 3 for the debit key).
     ...
     * @return The current instance.
     * @throw UnsupportedOperationException If the Change Key command is not
     * available for this card.
     * @throw IllegalArgumentException If the provided key index is out of
     * range.
