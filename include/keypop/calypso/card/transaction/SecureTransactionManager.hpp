...
public:
     ...
     * @since 2.0.0
     */
    virtual std::shared_ptr<CardTransactionCryptoExtension> getCryptoExtension()
        = 0;

    /**
     * Schedules the execution of a "Close Secure Session" command.
     *
     * <p>The ratification mechanism is disabled by default but can be enabled
     * via the {@link
     * SymmetricCryptoSecuritySetting#enableRatificationMechanism()} method.
     *
     * <p>In this case, a ratification command is added after the "Close Secure
     * Session" command when the communication is done in contactless mode.
     *
     * @return The current instance.
     * @throw IllegalStateException In the following cases:
     *     <ul>
     *       <li>No secure session is opened and no secure session opening is
     * prepared <li>A secure session closing is already prepared <li>A secure
     * session canceling is prepared
     *     </ul>
     *
     * @since 1.6.0
     */
     ...
     * order to abort the current secure session.
     *
     * <p>This command will be executed in safe mode and will not raise any
     * exceptions.
     *
     * @return The current instance.
     * @since 1.6.0
