 ...
 * API".
 *
 * <p>It is a dynamic view of the card's content, regularly updated from the
 ...
 * data read or modified during the execution of the processes defined by the
...
public:
    ...
    enum class ProductType {
        ...
        /**
         * Application type is equal to 0 or FFh or if the data provided by the
         * selection process is not properly formatted (bad TLV data).
         *
         * @since 1.0.0
         */
        UNKNOWN
     ...
     *
     * <p>The invalidation status is determined either from the response to the
     * Select Application command or from the response to a Select File (DF)
     * command.
     *
     * <p>For a PRIME_REVISION_3 card, a 6283h status word is returned in
     * response to the Select Application command when the corresponding DF is
     * invalidated.
     *
     * <p>For older Calypso cards it may be necessary to execute a Select File
     * command in order to determine the invalidation status.
     *
     ...
     *
     * <p>The AID selects the application by specifying all or part of the
     * targeted DF Name (5 bytes minimum).
     *
     * @return A not null byte array containing the DF Name bytes (5 to 16
     * bytes)
     * @since 1.0.0
     */
     ...
     *
     * @return A not null byte array containing the Application Serial Number (8
     * bytes).
     * @since 1.0.0
     */
    ...
    /**
     * Gets the Application Type byte which determines the Calypso product type
     * and various options.
     *
    ...
    /**
     * Gets the Application Subtype that provides a reference to the file
    ...
    /**
     * Gets the Software Issuer byte that indicates the entity responsible for
     * the software of the selected application.
     *
    ...
    /**
     * Gets the Software Version field that may be set to any fixed value by the
     * Software Issuer of the Calypso application.
     *
    ...
    /**
     * Gets the Software Revision field that may be set to any fixed value by
     * the Software Issuer of the Calypso application.
     *
     ...
     *
     * <p>Depending on the type of card, the session modification byte indicates
     * the maximum number of bytes that can be modified or the number of
     * possible write commands in a session.
     *
     ...
     *
     * <p>The returned array contains the software issuer ID and the
     * discretionary data.
     *
     ...
     *
     * <p>Note that if a secure session is actually running, then the object
     * contains all session modifications, which can be canceled if the secure
     * session fails.
     *
     * @param sfi The SFI to search.
     * @return Null if the requested EF is not found or if the SFI is equal to
     * 0.
     * @since 1.0.0
     */
    virtual const std::shared_ptr<ElementaryFile>
    getFileBySfi(const uint8_t sfi) const = 0;

    /**
     * Returns a reference to the ElementaryFile that has the provided LID
     * value.
     *
     * <p>Note that if a secure session is actually running, then the object
     * contains all session modifications, which can be canceled if the secure
     ...
     *
     * <p>Note that if a secure session is actually running, then the set
     * contains all session modifications, which can be canceled if the secure
     * session fails.
     *
     * @return A not null reference (it may be empty if no one EF is set).
    ...
    /**
     * Returns the transaction counter value provided in the output data of the
     * last successful "Open Secure Session" command.
     *
    ...
    /**
     * Indicates whether the Public Key Authentication is supported or not
     * (since rev 3.3).
     *
     ...
     *
     * <p>This boolean is initially the result of the interpretation of the
     * application type byte but may be updated after the secure session is
     * opened. Indeed, depending on the type of key used, the extended mode
     * functionalities may not be available (non-AES keys) and this information
    ...
    /**
     * Indicates if the PIN is blocked. The maximum number of incorrect PIN
    ...
    /**
     * Gives the number of erroneous PIN presentations remaining before
     * blocking.
     *
     ...
     * @see SecureSymmetricCryptoTransactionManager#prepareSvGet(SvOperation,
     * SvAction)
     * @see SecureSymmetricCryptoTransactionManager#prepareSvDebit(int)
     * @see SecureSymmetricCryptoTransactionManager#prepareSvDebit(int, byte[],
     * byte[])
     * @see SecureSymmetricCryptoTransactionManager#prepareSvReload(int)
     * @see SecureSymmetricCryptoTransactionManager#prepareSvReload(int, byte[],
     * byte[], byte[])
     * @since 1.0.0
     */
     ...
     * @since 1.0.0
     */
    virtual const std::shared_ptr<SvDebitLogRecord> getSvDebitLogLastRecord()
        = 0;

    /**
     * Gets list of references to the {@link SvDebitLogRecord} read from the
     * card.
     *
