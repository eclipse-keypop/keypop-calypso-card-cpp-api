...
public:
    ...
    /**
     * Adds a "Select File" command to select file according to the provided {@link
     * SelectFileControl} enum entry indicating the navigation case: FIRST, NEXT or CURRENT.
     *
     ...
     * @since 1.0.0
     */
    virtual CalypsoCardSelectionExtension& prepareSelectFile(const SelectFileControl selectControl)
        = 0;
     ...
     *
     * <p>Caution: the resulting APDU command is compliant with PRIME revision 3 cards. Therefore,
     * the command may be rejected by some earlier revision cards.
     *
     ...
     *
     * <p>Once this command is processed, the result is available in {@link CalypsoCard} if the
     * requested file exist in the file structure of the card and if the offset and number of bytes
     * to read are valid (best-effort mode).
     *
     * <p>Caution: the resulting APDU command is compliant with PRIME revision 3 cards. Therefore,
     * the command may be rejected by some earlier revision cards.
     *
     ...
     */
    virtual CalypsoCardSelectionExtension&
    prepareReadBinary(const uint8_t sfi, const int offset, const int nbBytesToRead)
        = 0;

    /**
     * Adds a "Read Records" command to read a part of a record of the indicated EF, which should be
     * a counter file.
     *
     ...
     * ticketing processing is done remotely in order to allow a complete secure session to be
     * carried out in a single exchange between the server and the terminal.
     *
     ...
     *   <li>the session contains commands that necessarily require exchanges with the card during
     *       the session (e.g. PIN, Stored Value, encryption, early mutual authentication, data not
     ...
     * mode. For cards that support this feature, this optimizes future exchanges with the card in
     * the case of secure sessions intended to be executed in a single step.
     *
     * <p>The objective of the pre-opening is to allow the grouping of all the commands of a secure
     * session. This functionality is only relevant in the case of a distributed system where the
     * ticketing processing is done remotely in order to allow a complete secure session to be
